...
INFLATION_CANDIDATE_PATHS = [
    DATA_DIR / "us_monthly_inflation_rates_1913_2026.csv",
...
def load_local_inflation() -> Tuple[pd.Series, Dict[str, object]]:
    ...
    if monthly_columns.issubset(frame.columns):
        ...
        if series.empty:
            raise ValueError(f"Parsed monthly inflation matrix is empty: {path}")

        metadata = {
            "path": str(path.relative_to(ROOT_DIR).as_posix()),
            "value_column": "monthly matrix Jan-Dec",
            "date_column": "Year + month columns",
            "frequency_label": "monthly",
            "resample_rule": "ME",
            "suggested_granger_lag": 12,
        }
