...
def plot_transformed_series(transformed_panel: pd.DataFrame) -> None:
    fig, axes = plt.subplots(transformed_panel.shape[1], 1, figsize=(14, 2.6 * transformed_panel.shape[1]), sharex=True)
    if transformed_panel.shape[1] == 1:
        axes = [axes]

    for ax, column in zip(axes, transformed_panel.columns):
        ax.plot(transformed_panel.index, transformed_panel[column], linewidth=1.0, color="#0b6e4f")
        ax.axhline(0.0, color="black", linewidth=0.8, alpha=0.6)
...
def plot_factor_vs_inputs(factor: pd.DataFrame, transformed_panel: pd.DataFrame) -> None:
    ...
    fig, ax = plt.subplots(figsize=(14, 6))
    ax.plot(merged.index, merged["latent_factor"], color="#7f1d1d", linewidth=2.2, label="Factor latente")
    for column in transformed_panel.columns:
        ax.plot(merged.index, merged[column], linewidth=0.9, alpha=0.65, label=column)
...
def plot_diagnostics(metadata: Dict) -> None:
    ...
    for ax, column in zip(axes, diagnostics_frame.columns):
        ax.barh(diagnostics_frame.index, diagnostics_frame[column], color=colors[column], alpha=0.85)
