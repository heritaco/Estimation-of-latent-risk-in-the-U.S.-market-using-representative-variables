...
def plot_factor_vs_pure_inflation(frame: pd.DataFrame) -> None:
    ...
    ax1.plot(
        ...
        color=PALETTE_3,
        linewidth=1.7,
        label="Factor latente (promedio mensual)",
    )
    ax2.plot(
        frame.index,
        ...
        color=PALETTE_7,
        linewidth=1.9,
        label="Índice de inflación pura (reindexado)",
    )

    ax1.set_title("Factor latente vs inflación pura")
    ax1.set_xlabel("Fecha")
    ax1.set_ylabel("Factor latente")
    ax2.set_ylabel("Índice de inflación (base 100)")
    style_grid(ax1)
...
def plot_scatter(frame: pd.DataFrame, corr_stats: Dict[str, float]) -> None:
    ...
    ax.set_title("Factor latente vs inflación pura")
    ax.set_xlabel("Factor latente (promedio mensual)")
    ax.set_ylabel("Índice de inflación (base 100)")
    style_grid(ax)
    style_text_box(
        ax,
        (
...
def plot_cross_correlation(cross_corr: pd.DataFrame) -> None:
    ...
    ax.set_title("Correlación cruzada: factor vs inflación pura")
